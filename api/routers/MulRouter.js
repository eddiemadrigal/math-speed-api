...
// protected
router.post('/multiply', checkAuth, (req, res) => {
    const data = req.body;
    MulProblems.add(data)
        .then (subProb => {
            res.status(200).json(subProb);
        })
        .catch (err => {
            res.status(500).json({ message: 'Failed to create a new mulitplication problem ...' })
        })
})
